...
ul3.append("li").append('button')
    ...
    .on('click', function(d){
      ...
      d3.select("#fromEdge").select('option').remove();
      d3.select("#toEdge").select('option').remove();
...
var selctTO = form.append("select").attr("id","toEdge");
...
nav.append("ul").append("li").append('button')
    ...
    .on("click", function (d) {
	    var selectEdgefrom = document.getElementById("fromEdge").value;
      var selectEdgeto = document.getElementById("toEdge").value;
...
var navtab = d3.select('body').append('center').append('div').append("table");
...
linetab.append("th").attr("class","tablamdaj").html("lamda(i) + V(i,j) = lamda(j)");
...
function isBefore(from, to) {
  for(var i = 0; i < from.anterieur.length; i++) {
    if(from.anterieur[i] == to) return true;
    else if(isBefore(from.anterieur[i], to)) return true;
  }
  return false;
}
...
if(d3.event.ctrlKey) {
    ...
    gSommet.append('text')
        ...
        d3.selectAll('option').remove();
        for (var i = tabsommet.length - 1; i >= 0; i--) {
          var affiche = tabsommet[i].id + 1;
        selctFROM.append("option").attr("value", tabsommet[i].id).html(affiche);
        selctTO.append("option").attr("value", tabsommet[i].id).html(affiche);
        }
...
function bellmanFord (listSommets, edges, source, destination) {
        ...
        o = 0; a = 0; 
          ...
          //fin initialisation valeurs lamda
           ...
           /***     var sommetadd;
                sommetadd = c.from.id;
                var curentSommet = c.to.id;

                while (sommetadd != null){
                  sommetadd = parents[sommetadd];
                  if (curentSommet == sommetadd){
                    console.log("bocle eeeee");
                    return;
                  }
                }   
            ***/

          d3.selectAll("#editable").remove();
            ...
            for (var j = 0; j < edges.length; j += 1) {
             ...
             // navtab.append("p").html(""+c.from.id+""+c.to.id+"");
              if (lamdas[c.to.id] - lamdas[c.from.id] > c.valeur_de_arc) {
                ...
                if(reseq>1000000000){reseq = "∞"; };


                linetab = navtab.append("tr").attr("id","editable");
                ...
                var fromi = c.from.id + 1;
                var toj = c.to.id + 1;
                var tmpres = lamdas[c.to.id] - lamdas[c.from.id];

                linetab = navtab.append("tr").attr("id","editable");
                linetab.append("td").attr("class","tabi").html(fromi);
                linetab.append("td").attr("class","tabj").html(toj);

                linetab.append("td").attr("class","tablamda").html(lamdas[c.to.id]+" - "+lamdas[c.from.id]+" = "+tmpres+" <= "+c.valeur_de_arc);
                linetab.append("td").attr("class","tabvij").html(c.valeur_de_arc);
          ...
          d3.selectAll('#node').append('text')
              ...
              .text(function(d){
                ...
                content = lamdas[ref]
                if(lamdas[ref] == 100000000000000000000){
                  content = "∞";
                }
                return "L = "+content+"";
  ...
  function constructTable(n, edges) {
    console.log('CONSTRUCTION DE LA TABLE');
    var table = [];
    for(var i = 0; i < n; i++) {
      table.push([]);
      for(var j = 0; j < n; j++)table[i].push(-1);
    }
    n = edges.length;
    for(var i = 0; i < n; i++)table[edges[i].from.id][edges[i].to.id] = edges[i].to.id;
  ...
  function detectBoucle(tab) {
    console.log('DETECTION DE BOUCLE');
    var av = [];
    var ap = [];
    ...
    for(var i = 0; i < n; i++) {
      ...
      ap = [];
      for(var j = 0; j < n; j++) {
        if(tab[i][j] != 0)ap.push(j);
        if(tab[j][i] != 0)av.push(i);
      }

      for(var j = 0; j < av.length; j++)for(var k = 0; k < ap.length; k++) {
          printTable(tab);
        tab[av[j]][ap[k]] = tab[av[j]][i];
        if(tab[ap[k]][av[j]] != -1) {
          console.log('Boucle détecté');
          //console.log('Boucle détecté'' : '+i+', '+tab[av[j]][i]);
          return true;
...
function bellmanFordMAX (listSommets, edges, source, destination) {
        ...
        var t = constructTable(listSommets.length, edges);
        printTable(t);
        if(detectBoucle(t)) return;
